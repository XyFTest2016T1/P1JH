...
class Psse:
    '''In physical units, i.e. data convention, i.e. PSSE data files'''
...
class Rop:
    ...
    def trancostfuncfrom_phase_0(self,rawdata):
        ...
        for r in rawdata.generators.values():
            ...
            for i in range(self.active_power_dispatch_records.get((r.i,r.id)).npairs):
                ...
                self.active_power_dispatch_records.get((r.i,r.id)).points.append(self.active_power_dispatch_records.get((r.i,r.id)).constc+self.active_power_dispatch_records.get((r.i,r.id)).linearc*(r.pb+i*(r.pt-r.pb)/10)+self.active_power_dispatch_records.get((r.i,r.id)).quadraticc*pow(r.pb+i*(r.pt-r.pb)/10,2))
    ...
    def write(self, file_name,rawdata):
        ...
        file = open("C:\SyncDrive\PNNL_Project\ACOPF Competition\eval_phase1\\testfile.rop","w") 
        file.write(" 0          /  PSS/E-30.0, MON, SEP 20 2004  17:32\n") 
...
class GeneratorDispatchRecord:
    ...
    def read_from_row(self, row):
        ...
        self.genid = parse_token(row[1], str, '1')
